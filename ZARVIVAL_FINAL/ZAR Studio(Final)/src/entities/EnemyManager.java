...
public class EnemyManager {
	...
	private void drawSharks(Graphics g, int xLvlOffset) {
		for (Shark s : currentLevel.getSharks())
			if (s.isActive()) {
				g.drawImage(sharkArr[s.getState()][s.getAniIndex()], (int) (s.getHitbox().x + 10) - xLvlOffset - SHARK_DRAWOFFSET_X + s.flipX(),
						(int) s.getHitbox().y - SHARK_DRAWOFFSET_Y + (int) s.getPushDrawOffset() + s.flipW(), SHARK_WIDTH * s.flipW(), SHARK_HEIGHT, null);
//				s.drawHitbox(g, xLvlOffset);
//				s.drawAttackBox(g, xLvlOffset);
			}
	}

	private void drawArmadillo(Graphics g, int xLvlOffset) {
		for (Pinkstar p : currentLevel.getPinkstars())
			if (p.isActive()) {
				g.drawImage(pinkstarArr[p.getState()][p.getAniIndex()], (int) p.getHitbox().x - xLvlOffset - PINKSTAR_DRAWOFFSET_X + p.flipX() - 20,
	...
	private void drawCrabs(Graphics g, int xLvlOffset) {
		for (Crabby c : currentLevel.getCrabs())
			if (c.isActive()) {
				...
				g.drawImage(crabbyArr[c.getState()][c.getAniIndex()], (int) c.getHitbox().x - xLvlOffset - MUSHROOM_DRAWOFFSET_X + c.flipX() ,
						(int) c.getHitbox().y - MUSHROOM_DRAWOFFSET_Y + (int) c.getPushDrawOffset() -52, MUSHROOM_WIDTH * c.flipW(), MUSHROOM_HEIGHT, null);
...
//				c.drawAttackBox(g, xLvlOffset);
	...
	private void loadEnemyImgs() {
		crabbyArr = getImgArr(LoadSave.GetSpriteAtlas(LoadSave.MUSHROOM_SPRITE), 15, 5, MUSHROOM_WIDTH_DEFAULT , MUSHROOM_HEIGHT_DEFAULT);
