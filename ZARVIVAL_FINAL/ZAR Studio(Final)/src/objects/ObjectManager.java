...
public class ObjectManager {
	...
	private void loadImgs() {
		...
		cannonImgs = new BufferedImage[11];
		BufferedImage temp = LoadSave.GetSpriteAtlas(LoadSave.CANNON_ATLAS);

		for (int i = 0; i < cannonImgs.length; i++)
			cannonImgs[i] = temp.getSubimage(i * 64, 0, 64, 32);

		cannonBallImg = LoadSave.GetSpriteAtlas(LoadSave.CANNON_BALL);
		treeImgs = new BufferedImage[2][23];
		BufferedImage treeOneImg = LoadSave.GetSpriteAtlas(LoadSave.TREE_ONE_ATLAS);
		for (int i = 0; i < 16; i++)
			treeImgs[0][i] = treeOneImg.getSubimage(i * 53, 0, 53, 96);

		BufferedImage treeTwoImg = LoadSave.GetSpriteAtlas(LoadSave.TREE_TWO_ATLAS);
		for (int i = 0; i < 23; i++)
			treeImgs[1][i] = treeTwoImg.getSubimage(i * 74, 0, 74, 128);
	...
	public void drawBackgroundTrees(Graphics g, int xLvlOffset) {
		for (BackgroundTree bt : currentLevel.getTrees()) {
			...
			if (type == 9)
				type = 8;
			g.drawImage(treeImgs[type - 7][bt.getAniIndex()], bt.getX() - xLvlOffset + GetTreeOffsetX(bt.getType()) - 5, (int) (bt.getY() + GetTreeOffsetY(bt.getType())) - 44, GetTreeWidth(bt.getType()),
	...
	private void drawCannons(Graphics g, int xLvlOffset) {
		for (Cannon c : currentLevel.getCannons()) {
			...
			g.drawImage(cannonImgs[c.getAniIndex()], x, (int) (c.getHitbox().y) - 10, width, CANNON_HEIGHT, null);
		}
	}
